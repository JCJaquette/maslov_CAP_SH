...
function [verif,r,vali_data] = verify_homoclinic_orbit(params, mflds, x, nu)%remove vali_data if things work
    ...
    % B3: use a FIXED double approximate inverse A = inv(mid(DF)) as the
    % radii-polynomial operator, instead of a verified interval inverse
    % DF^(-1).  This is the standard rigorous choice -- A may be any fixed
    % bounded operator, and Z0 = ||I - A*DF|| (with DF interval) certifies it.
    % It is far faster (one double inverse vs a verified interval inverse of
    % a ~4m x 4m matrix) and gives tighter bounds.
    if params.isIntval
        Am = inv(mid(DF));
    else
        Am = DF^(-1);
