function [Y,Z, Z0] = get_radii_poly_coeffs(nu,x,mflds,params,DF,Am)
    ...
    z_inf(4)=(x.Lbvp/(2*m))*(nu+1/nu);
    z_inf(5)=(x.Lbvp/(2*m))*(nu+1/nu);
    z_inf(6)=(x.Lbvp/(2*m))*(nu+1/nu);
    z_inf(7)=(x.Lbvp/(2*m))*(nu+1/nu)*(8*params.nu*norms.a1+48*norms.a1^2 +3 + params.mu);
    ...
    [a1vI]=chebstar2(abs(x.a1),vI, 3*m);
    [a1vIvI]=chebstar2(abs(a1vI),vI, 3*m); 
    ...
    Z=zero*zeros(7,3);
     ...
     Z(1:3,1)=cte_s*sum(norms.A(1:3,1:3),2)+cte_u*sum(norms.A(1:3,4:7),2)+...
        +x.Lbvp*abs(Am(1:3, 3+3*m+1:3+4*m))*(2*params.nu*a1vI(1:m)'+...
      ...
      Z(4,1)=cte_s*sum(norms.A(4,1:3),2) ... 
        ...
        +x.Lbvp*abs(Am(4, 3+3*m+1:3+4*m))*((2*params.nu*a1vI(1:m)'+...
    ...
    Z(5,1)=cte_s*sum(norms.A(5,1:3),2) ... 
        ...
        +x.Lbvp*abs(Am(3+m+1, 3+3*m+1:3+4*m))*((2*params.nu*a1vI(1:m)'+...
        2*params.nu*a1vI(3:m+2)' + 3*a1vIvI(1:m)' +3*a1vIvI(3:m+2)').*nu_power')...
        + z_inf(5);
     ...
     Z(6,1)=cte_s*sum(norms.A(6,1:3),2) ... 
        ...
        +x.Lbvp*abs(Am(3+2*m+1, 3+3*m+1:3+4*m))*((2*params.nu*a1vI(1:m)'+...
        2*params.nu*a1vI(3:m+2)' + 3*a1vIvI(1:m)' +3*a1vIvI(3:m+2)').*nu_power')...
        + z_inf(6);
     ...
     Z(7,1)=cte_s*sum(norms.A(7,1:3),2) ... 
        ...
        +x.Lbvp*abs(Am(3+3*m+1, 3+3*m+1:3+4*m))*((2*params.nu*a1vI(1:m)'+...
        2*params.nu*a1vI(3:m+2)' + 3*a1vIvI(1:m)' +3*a1vIvI(3:m+2)').*nu_power')...
        + z_inf(7);
   ...
   Z(1:7,2)=norms.A(1:7,1)*LAMBDA_S_1+norms.A(1:7,2)*LAMBDA_S_2+norms.A(1:7,3)*LAMBDA_S_4+...
        ...
        +norms.A(1:7,7)*(LAMBDA_U_4+x.Lbvp*(nu+1/nu)*(8*params.nu+256*norms.a1));

   Z(1:7,3)=norms.A(1:7,7)*48*x.Lbvp*(nu+1/nu);
