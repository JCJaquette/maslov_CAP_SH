function [output] = Cauchy_Mat_rep(Coeff_in,N_out)
...
toep_depth = N_inner+N_out+1;
toep_width = N_out+1;


output = zeros(toep_depth*(N_inner+1+N_out),toep_width^2);

% Toeplitz_matrix_list = zeros()

% Make the a column of N, NxN matrixes given by Toepliz Matrices
toep_row_top = 0*(1:toep_width);
toep_col = 0*(1:toep_depth);
for i = 1:N_inner+1
    toep_col(1:N_inner+1) = Coeff_in(:,i);
...
Mat_column = output(1:toep_depth*(N_inner+1),1:toep_width);


for i = 2:N_out+1 
    N_shift_down = (i-1)*toep_depth;
    N_shift_right = (i-1)*toep_width;
    output(N_shift_down +1:N_shift_down+toep_depth*(N_inner+1),N_shift_right+1:N_shift_right+toep_width) = Mat_column ;
end
