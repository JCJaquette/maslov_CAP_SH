...
defect=conv2(W_coeff_inv,W_coeff);
defect(1,1)=defect(1,1)-1;
disp('Inverse Defect')
sum(abs(defect),'all')
toc
...
if BOOL_A_inverse
    disp('Computing A_Inverse')
    m_inv = (N_inverse+1)*(N_inverse+2)/2-1;

    A_dagger = zeros(m_inv,m_inv);
    m_counter=0;

    for alphaNorm=1:(N_inverse)    
        for i=0:alphaNorm
            ...
            % LM_local = conv2(W_coeff, L_deriv.*U_coeff_local)+M_operator( U_coeff_local, W_coeff,K,lambda_s_1);
            % LM_local=LM_local(1:N+1,1:N+1);
            % x_local = compress(LM_local,N_inverse);        
            % A_dagger(:,m_counter)=x_local;
            ...
            x_local = compress(next_trunc,N_inverse);        
            A_dagger(:,m_counter)=x_local;            
        end
    end

end
B_dagger =eye(m_inv,m_inv)+A_dagger;
B=inv(B_dagger);
% A=inv(A_dagger);

Z_0_mat = intval(B)*intval(B_dagger)-eye(m_inv);
Z_0 = norm(Z_0_mat ,1)

disp('norm of B^{dagger}, B')
norm(B_dagger,1)
norm(B,1)
% Z_0_mat = intval(A)*intval(A_dagger)-eye(m_inv);
% Z_0 = norm(Z_0_mat ,1)
return
    % 
% 
...
for k=1:floor(N)
    ...
    % L_part = conv2(L_deriv.*U_coeff,W_coeff);
    % L_part = conv2(L_part,W_coeff_inv);
    % L_part=L_deriv_inv.*L_part(1:N+1,1:N+1);
    % L_part = U_coeff-L_part;
    % L_part(1,1)=0;
    % disp(['L defect =', num2str(norm(L_part,1))])
    % next_trunc =next_trunc+L_part;
...
% 
prod = ifft2(fft2(A_pad).*fft2(B_pad));
prod_trunc = prod(1:N-1,1:N-1);

end

function out = M_operator( U, W,K,lambda)

% B = 4*Cauchyfft2(K.*W,K.^2.*U);
% C = Cauchyfft2(4*lambda*K.*W+6*K.^2.*W,K.*U);
% D = 4*Cauchyfft2((1+lambda^2)*K.*W+K.^3.*W,U);
...
function x = compress(A,m)
    x=0*(1:(m*(m+1)/2-1))';
    m_counter = 0;
    for alphaNorm=1:m
        for i=0:alphaNorm
            m_counter = m_counter +1;
            x(m_counter)=A(1+i,alphaNorm-i+1);
        end
    end
end

function A = expand(x,N,m)
    A =zeros(N,N);
    for i=1:m
        x_cut = x(1:m-i+1);        
        A(1:m-i+1,i)=x_cut;
